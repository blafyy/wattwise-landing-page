...
function doLogin(){
  ...
  if(!ok)return;
  closeModal();toast('¡Bienvenido de vuelta! Redirigiendo al dashboard…');
}
...
function doRegister(){
  ...
  if(!ok)return;
  if(!document.getElementById('rT').checked){toast('Debes aceptar los términos');return;}
  closeModal();toast('¡Cuenta creada! Revisa tu correo para confirmar.');
}

function socialLogin(p){closeModal();toast('Conectando con '+p+'…');}
...
function submitContact(){
  ...
  if(!ok)return;
  if(!document.getElementById('cT').checked){toast('Debes aceptar los términos');return;}
